...
class TestComment(BaseTestArticles):
    ...
    def test_failed_to_update_comment(self):
        ...
        resp = self.client.put(
            '/api/articles/comments/{}'.format(comment_id),
            data, format='json')
    ...
    def test_user_try_to_delete_a_comment_which_exist(self):
        comment_id = 10000
        self.client.credentials(
            HTTP_AUTHORIZATION='Bearer ' + self.get_samantha_token())
        resp = self.client.delete(
        ...
        self.assertIn(
            "Failed, comment or article doesnot exist",
            str(resp.content))
    ...
    def test_user_successfully_reply_to_a_comment(self):
        data = {
            "reply": {
	            "reply_body": "This is the reply body"
                }
	        }
        self.client.credentials(
            HTTP_AUTHORIZATION='Bearer ' + self.login_user())
        ...
        resp = self.client.post(
            '/api/articles/{}/reply/'.format(comment_id),
            data, format='json')
        self.assertEqual(res.status_code, 201)
        self.assertIn("reply_body", str(resp.content))
    ...
    def test_user_try_to_reply_on_a_comment_which_doesnt_exist(self):
        ...
        resp = self.client.post(
            ...
            data, format='json')
        self.assertEqual(resp.status_code, 400)
        self.assertIn("Comment not found", str(resp.content))

    def test_user_successfully_update_his_reply_on_a_comment(self):
        data = {
            "reply": {
	            "reply_body": "This is the reply body"
                }
        ...
        update_data = {
            "reply": {
                "comment": comment_id,
	            "reply_body": "This is an update"
                }
	        }
        response = self.client.put(
            '/api/articles/comments/reply/{}'.format(reply_id),
            update_data, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertIn("This is an update", str(response.content))

    def test_user_try_to_update_reply_that_doesnt_exist(self):
        data = {
            "reply": {
	            "reply_body": "This is the reply body"
                }
        ...
        self.assertIn("can't update, reply not found", str(response.data))

    def test_user_can_only_update_his_reply(self):
        data = {
            "reply": {
	            "reply_body": "This is the reply body"
                }
        ...
        self.assertIn("You dont have permission to perform this action", str(response.content))

    def test_user_successfully_delete_his_reply_from_the_comment(self):
        data = {
            "reply": {
	            "reply_body": "This is the reply body"
                }
        ...
        response = self.client.delete(
            ...
            update_data, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertIn("Reply deleted successfully", str(response.content))

    def test_cant_delete_a_non_existing_reply(self):
        data = {
            "reply": {
	            "reply_body": "This is the reply body"
                }
        ...
        resp = self.client.post(
            ...
            data, format='json')
        response = self.client.delete(
            '/api/articles/comments/reply/1000',
            format='json')
        self.assertEqual(response.status_code, 400)
        self.assertIn("Can't delete, reply does not exist", str(response.data))

    def test_user_can_only_delete_his_reply(self):
        data = {
            "reply": {
	            "reply_body": "This is the reply body"
                }
